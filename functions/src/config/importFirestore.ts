...
export async function importJsonToFirestore() {
  try {
    ...
    if (modelsSnapshot.empty || brandsSnapshot.empty) {
      ...
      const updatedModels = modelsData.map((car: any) => {
        // Assign a new brand ID if the brand name is not already in the dictionary
      ...
      // Create a dictionary to store brand IDs and their corresponding average prices
      const brandPrices: Record<number, number[]> = {};

      // Populate the brandPrices dictionary with the average prices of each model
      updatedModels.forEach((model: Model) => {
        if (!brandPrices[model.brand_id]) {
      ...
      brandsList.forEach((brand: Brand) => {
        const docRef = db.collection("brands").doc(brand.brand_id.toString());
